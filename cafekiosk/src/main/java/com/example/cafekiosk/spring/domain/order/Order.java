...
// sql 예약어 때문에 order를 사용할 수 없음, 따라서 실제 테이블 적용 시에는 orders로 적용
@Table(name = "orders")
@Getter
public class Order extends BaseEntity {
    ...
    private Order(List<Product> products, LocalDateTime registeredDateTime) {
        ...
        this.orderProducts = createOrderList(products);
    }

    private int calculateTotalPrice(List<Product> products) {
        return products.stream()
                .mapToInt(product -> product.getProductInfo().getPrice())
                .sum();
    }

    private List<OrderProduct> createOrderList(List<Product> products) {
        return products.stream()
